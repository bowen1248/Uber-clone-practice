    ...
    var tripCancelledMessage: String {
        ...
        if user.accountType == .passenger {
            if trip.state == .driverCancelled {
                ...
                return "You trip has been cancelled"
            }
        } else {
            if trip.state == .driverCancelled {
                return "You trip has been cancelled"
    ...
    func fetchUser() {
        service.$user
            .sink { user in
                ...
                if user.accountType == .passenger {
                    print("DEBUG: Fetch Drivers")
                    self.fetchDrivers()
                    self.addTripObserverForPassenger()
                } else {
                    print("DEBUG: Fetch Trips")
...
extension HomeViewModel {
    ...
    func requestTrip() {
        ...
        print("DEBUG: Customer is \(currentUser.fullname)")
        ...
        Firestore.firestore().collection("trips")
            ...
            .addSnapshotListener { snapshot, _ in
                ...
                self.getDestinationRoute(from: trip.driverLocation.toCoordinate(),
                    ...
                    print("DEBUG: Travel time to passenger is \(Int(route.expectedTravelTime / 60))")
    ...
    func selectLocation(_ localSearch: MKLocalSearchCompletion, config: LocationResultsViewConfig) {
        locationSearch(forLocalSearchCompletion: localSearch) { response, error in
            ...
            case .saveLocation(let viewModel):
                ...
                print("DEBUG: Saved Location coordinates \(coordinate)")
            }
        }
    }
